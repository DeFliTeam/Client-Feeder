...
if [ -f /boot/defli-config.txt ]; then
    UUID_FILE="/boot/defli-uuid"
else
    mkdir -p /usr/local/share/defliteam
    UUID_FILE="/usr/local/share/defliteam/defli-uuid"
    # move old file position
    if [ -f /boot/defli-uuid ]; then
        mv -f /boot/defli-uuid $UUID_FILE
    fi
fi
